{
  ...
  xdg.configFile."reclone.yaml" = {
    enable = true;
    target = "ghorg/reclone.yaml";
    text = ''
      # +-+-+-+-+-+-+-+-+-+-+-+-+-+
      # |G|H|O|R|G| |R|E|C|L|O|N|E|
      # +-+-+-+-+-+-+-+-+-+-+-+-+-+
      gitlab-yggdrasil:
        cmd: "ghorg clone yggdrasil3 --scm=gitlab --preserve-dir --token ${config.age.secrets.ghorg-gitlab-com-token.path} --output-dir gitlab.com/yggdrasil3"
        description: "Clones the Yggdrasil GitLab group"
      gitlab-dvonessen:
      ...
      github-dvonessen:
        cmd: "ghorg clone dvonessen --scm=github --clone-type user --preserve-dir --token ${config.age.secrets.ghorg-github-com-token.path} --output-dir github.com/dvonessen"
        description: "Clones the dvonessen user repositories"
      github-gcixdev:
        cmd: "ghorg clone gcixdev --scm=github --preserve-dir --token ${config.age.secrets.ghorg-github-com-token.path} --output-dir github.com/gcixdev"
        description: "Clones the gcixdev org repositories"
      dbsystel-serviceteam:
        cmd: "ghorg clone serviceteamdbv2 --scm=gitlab --preserve-dir --base-url=https://git.tech.rz.db.de --token ${config.age.secrets.ghorg-dbsystel-gitlab-token.path} --output-dir git.tech.rz.db.de/serviceteamdbv2"
        description: "Clones the gcixdev org repositories"
      dbsystel-free4all:
        cmd: "ghorg clone free4all --scm=gitlab --preserve-dir --base-url=https://git.tech.rz.db.de --token ${config.age.secrets.ghorg-dbsystel-gitlab-token.path} --output-dir git.tech.rz.db.de/free4all"
        description: "Clones the gcixdev org repositories"
    '';
  };

  programs.bash.initExtra = ''
    source <(${pkgs.ghorg}/bin/ghorg completion bash)
  '';
  programs.zsh.initExtra = ''
    source <(${pkgs.ghorg}/bin/ghorg completion zsh)
  '';

  age.secrets."ghorg-gitlab-com-token" = {
    file = ../../agenix/ghorg-gitlab-com-token.age;
    path = "${config.xdg.configHome}/ghorg/gitlab-com-token";
  };
  age.secrets."ghorg-dbsystel-gitlab-token" = {
    file = ../../agenix/ghorg-dbsystel-gitlab-token.age;
    path = "${config.xdg.configHome}/ghorg/dbsystel-gitlab-token";
  };
  age.secrets."ghorg-github-com-token" = {
    file = ../../agenix/ghorg-github-com-token.age;
    path = "${config.xdg.configHome}/ghorg/github-com-token";
  };
}
